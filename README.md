...
Titan 2 is a great phone with a great keyboard, but the support for input languages is very limited. However, it allows showing on-screen keyboards and switching between them using the globe icon in the bottom-right corner.

Unexpected Keyboard allows custom layouts - so in this case the keyboard only shows characters that are **not** available directly on the physical keyboard. This means that there is an additional row of virtual keys directly above the keyboard, very easy to reach and easy to operate.
